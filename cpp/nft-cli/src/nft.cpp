...
namespace nft
{
    ...
    Task<void> Client::initialize()
    {
        try
        {
            co_await m_violas_client->await_allow_custom_script(true);
        ...
        try
        {
            m_violas_client->create_parent_vasp_account(admin.address, admin.auth_key, "nft Admin", true);
        ...
        auto [sender, sn] = co_await m_violas_client->await_execute_script(0, INITIALIZE_SCRIPT_FILE, {}, {});
        co_await m_violas_client->await_check_txn_vm_status(sender, sn, "nft::intialize");
        cout << "Succeeded to execute nft::initialize() ." << endl;
    }
