...
namespace Violas {
            ...
            if (!ret) {
                throw runtime_error(
                        format("failed to get tansaction and events by range, %d ",
                               get_last_error().c_str()) + EXCEPTION_AT);
            }
    ...
    public:
        ...
        virtual void publish(uint64_t account_index) override {
            ...
            m_libra_client->execute_script(account_index, (script_name += ".mv").c_str(),
                                           vector < string > {});
        }
    ...
    protected:
        void init_all_script(const std::string &script_files_path) {
            ...
            m_temp_path =
                    fs::temp_directory_path() / uint256_to_string(m_governor_addr); ///tmp/xxxxx
